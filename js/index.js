...
const sumDigits = (number) => {
  ...
  return (
    Number(numberToString.charAt(0)) +
    Number(numberToString.charAt(1)) +
    Number(numberToString.charAt(2))
  );
};
...
const longestWordArray = (array) => {
  ...
  } else if (array.length[1] > array.length[2] && array.length[0]) {
    console.log(array.length[1]);
  } else if (array.length[2] > array.length[1] && array.length[0]) {
    console.log(array.length[2]);
  }
};
const longestWordArrayResult = longestWordArray(["oso", "paloma", "ir"]);
...
const onlyVowelsResult = onlyVowels("hola");
